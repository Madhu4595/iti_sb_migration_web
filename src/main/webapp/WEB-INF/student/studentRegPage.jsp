...
function validate() {
	...
	if(ssc_regno == "" || ssc_regno == null){ $("#ssc_regnoError").html("Hall Ticket Number is required please enter."); $("#ssc_regnoError").css({"color": "red"}); $("#ssc_regnoError").focus(); return false; }
	if (!ssc_regno.match('^[0-9A-Z]{6,15}$')) { $("#ssc_regnoError").html("Hall Ticket Number is in invalid format."); $("#ssc_regnoError").css({"color": "red"}); $("#ssc_regnoError").focus(); return false; }
			...
			<div class="row m-1">
				<div class="col-12 text-center">
					<h3 class="h3" style="color: blueviolet; font-size: 15px;">
						Registration Form for Admission into the industrial training
						Institute for the session 2023(Phase-2)</h3>
				</div>
			</div>
